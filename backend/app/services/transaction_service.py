...
def _customer_stats(db: Session, account: Account) -> dict:
    ...
    avg_amount = (
        ...
        if amounts
        else float(account.balance) * 0.1 or 1000.0
    )
...
async def process_deposit(
    ...
    result = analyze_transaction(
        amount=float(amount),
        avg_txn_amount=stats["avg_amount"],
        txn_frequency_24h=stats["freq_24h"],
        is_new_beneficiary=False,
        prior_suspicious_count=stats["prior_suspicious"],
...
async def process_transfer(
    ...
    is_new_beneficiary = (
        datetime.utcnow() - beneficiary.created_at
    ) < timedelta(hours=24)
...
async def _finalize(
    ...
    if result["risk_score"] >= settings.RISK_HOLD_THRESHOLD:
        ...
        await notify_user(
            ...
            owner.id,
            "TRANSACTION",
            f"{txn.type.value.title()} Successful",
        ...
        if owner.email:
            ...
            send_email(
                owner.email,
                f"BankShield AI: {txn.type.value.title()} Successful",
                f"""Hello {owner.full_name},

Your {txn.type.value.lower()} of ₹{txn.amount:,.2f} was completed successfully.

Transaction Reference: {txn.reference}
Risk Score: {txn.risk_score}
Risk Level: {txn.risk_level.value if hasattr(txn.risk_level, "value") else txn.risk_level}

Thank you for using BankShield AI.
"""
            )

    _update_risk_profile(
        db,
        owner.id,
        txn
    )
...
def _settle(
    ...
    if txn.type == TransactionType.DEPOSIT:
        ...
        primary_account.balance = (
            Decimal(primary_account.balance)
            + Decimal(txn.amount)
        )
        ...
        if txn.receiver_account_id:
            ...
            receiver: Account = db.query(Account).get(
                txn.receiver_account_id
            )

            receiver.balance = (
                Decimal(receiver.balance)
                + Decimal(txn.amount)
            )


async def process_bank_cash_deposit(
    db: Session,
    account: Account,
    amount: Decimal,
    banker: User
    ...
    if account.status.value != "ACTIVE":
        raise HTTPException(
            status_code=400,
            detail=f"Account is {account.status.value}, cannot transact"
        )
    ...
    txn = Transaction(
        ...
        fraud_probability=Decimal("0"),
        risk_reasons=[
            "Branch cashier operation — fraud scoring bypassed"
        ],
    ...
    if owner.email:
        ...
        send_email(
            owner.email,
            "BankShield AI: Cash Deposit Completed",
            f"""Hello {owner.full_name},

A branch cash deposit of ₹{amount:,.2f} was successfully posted to your account.
...
async def process_bank_cash_withdrawal(
    db: Session,
    account: Account,
    amount: Decimal,
    banker: User
    ...
    if owner.email:
        ...
        send_email(
            owner.email,
            "BankShield AI: Cash Withdrawal Completed",
            f"""Hello {owner.full_name},

A branch cash withdrawal of ₹{amount:,.2f} was successfully posted to your account.
